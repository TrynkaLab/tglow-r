  ...
  if (mode == "merge") {
    for (assay in names(a@assays)) {
      if (assay %in% b.assays) {
        ...
        a@assays[[assay]]@data       <- TglowMatrix(cbind(a@assays[[assay]]@data, bb@data))
        ...
        features[features.b, colnames(b@assays[[assay]]@features)] <- b@assays[[assay]]@features[features.b,]
        a@assays[[assay]]@features   <- features
        
        
        if (!is.null(a@assays[[assay]]@scale.data)) {
          if (!is.null(b@assays[[assay]]@scale.data)) {
            a@assays[[assay]]@scale.data <- TglowMatrix(cbind(a@assays[[assay]]@scale.data, b@assays[[assay]]@scale.data[nearest.n[,1], features.b, drop=FALSE]))
          } else {
