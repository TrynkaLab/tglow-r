...
#' @param pc.max The maximum number of components to calculate using \code{\link{rlba::prcomp_irlba}}
...
#' @export
find_outliers_pca <- function(dataset, assay, qc.group = NULL, thresh = 3.5, pc.thresh = 0.75, pc.max = 500, pc.n = NULL, slot = "data", method = "z", return.pcs = FALSE, rfast.zerotol = 1e-10) {
    ...
    for (group in unique(qc.group)) {
        ...
        if (method == "mod.z") {
            pcs.norm <- apply(pca$x[, 1:pc.n, drop = F], 2, mod_zscore)
        } else if (method == "z") {
            pcs.norm <- fast_colscale(pca$x[, 1:pc.n, drop = F])
            # pcs.norm <- apply(pca$x, 2, scale, center = T, scale = T)[, 1:pc.n, drop = F]
        } else {
            stop(paste0("Method ", method, " is not available. Must be 'z' or 'mod.z'"))
        }

        outliers <- rowSums(abs(pcs.norm) < thresh) != pc.n
...
#' @returns Logical indicating if object is an outlier in pca space, or a list if return.pcs=TRUE
#' @export
find_outliers_pca_fixed <- function(dataset, assay, thresh = 3.5, qc.group = NULL, pc.n = 5, slot = "data", method = "z", return.pcs = FALSE) {
    if (assay %in% c("image.data", "image.data.trans", "image.data.norm")) {
        cur.assay <- slot(dataset, assay)
    } else {
        cur.assay <- dataset[[assay]]
